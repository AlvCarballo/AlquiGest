          ...
          <tbody>
            ${fincas.length ? (() => {
              ...
              return fincasPag.map(f => {
                ...
                <td class="td-actions">
                  <button class="btn btn-sm btn-secondary" title="Ver pisos" onclick="navigate('inmuebles',{finca_id:${f.id},titulo:'Pisos – ${(f.nombre||f.calle||'').replace(/'/g,"\\'")}'})" style="font-size:11px">Pisos</button>
                  <button class="btn btn-sm btn-success" title="Ver recibos" onclick="navigate('recibos',{finca_id:${f.id},titulo:'Recibos – ${(f.nombre||f.calle||'').replace(/'/g,"\\'")}'})" style="font-size:11px">Recibos</button>
                  <button class="btn btn-sm btn-secondary btn-icon" title="Editar" onclick="modalFinca(${f.id})">
                    <svg width="14" height="14" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                  </button>
                  ${_cfgVisi('VisiBorrarFinc') ? `<button class="btn btn-sm btn-danger btn-icon" title="Eliminar" onclick="deleteFinca(${f.id})">
